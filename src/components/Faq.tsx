...
const QA = [
  ...
  },
  {
    ...
    // ⚠️ 「운영자 두 명」에서 수를 뺐다(2026-08-25, 팀원 합류). 명단은 세 명이 됐지만
    //    **현장에 몇 명이 상주하는지는 확정된 바가 없다.** 수는 신뢰 신호라 되살릴
    //    가치가 있으니, 확정되면 「운영자 O명이」로 다시 넣을 것.
    a: "신청 시 신분증으로 나이를 확인하고, 확인이 끝나면 사본은 보관하지 않습니다. 행사 시간 동안 공간을 통째로 빌리기 때문에 모르는 손님과 섞이거나 대화가 옆자리에 들릴 일이 없습니다. 운영자가 현장에 상주하며 진행하고, 대화는 정해진 순서대로만 이루어집니다. 행사 중 연락처 교환은 강요되지 않습니다.",
  },
  {
    q: "술을 마시나요?",
        ...
        <div className="faq__list stagger">
          ...
          <details className="faq__item" data-reveal>
            ...
            <div data-answer>
              ...
              {/* 🔴 법정 청약철회권. 위 3단(행사일 기준)과 축이 다르므로 반드시 함께 뜬다.
                  ⚠️ .refund__law는 --mute로 낮추지 말 것 — 소비자에게 **유리한** 조항을
                     흐리게 두면 "숨겼다"는 항변을 만든다. 근거는 lib/event.ts REFUND_LAW. */}
              <div className="refund__law">
                {REFUND_LAW.map((line) => (
                  <p key={line}>{line}</p>
                ))}
              </div>
