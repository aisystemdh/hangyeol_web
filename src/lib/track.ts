 ...
 *
 * 왜 필요한가: 홈이 온보딩 무대(게이트 → 질문 3개 → 허브 → 신청)라, 어느 질문에서 사람이
 ...
 *
 * `q2_answer`가 이 목록에서 가장 값나가는 이벤트다 — 2번 질문은 **1번에서 같은 답을 고른
 * 사람에게 그 이유**를 묻는다. 그 분포가 흩어져 있다는 것이 곧 "답이 같아도 이유는 다르다"는
 * 브랜드 명제의 실측 증거가 되고, 그대로 통계 콘텐츠의 원본 집계가 된다.
 * (props의 `after`가 1번 답이라, 답별로 이유 분포를 나눠 볼 수 있다.)
 *
 * `gate_start` : `gate_skip` 비율이 그다음이다 — 온보딩을 할 의향의 유일한 실측치다.
 * 건너뛰기가 압도적이면 게이트 한 줄이 약한 것이지 온보딩이 나쁜 게 아니다.
 * `hero_start`는 건너뛴 사람이 히어로 CTA로 되돌아온 수 — 그 CTA의 값을 이 숫자가 정한다.
 *
 * ⚠️ 옛 `q3_reason`은 삭제됐다(9차 구조). 이유가 마지막 질문의 부록이 아니라
 *    **두 번째 질문 자체**가 되면서 `q2_answer`가 그 자리를 대신한다.
 */
