...
- 🔴 `docs/decisions/003-scenario-redesign-2026-09-05.md` — 지금 서 있는 구조의 정본.
  선착순 슬롯이 사라지고 상태가 3개가 되며, **자리는 「입금 확인」 순간에만 찬다.**
- 🔴 `docs/decisions/004-aligo-cumulative-mypage-2026-09-11.md` — **앞으로 만들 구조의 정본.**
  003의 네 곳을 대체한다 — 대행사(뿌리오→**알리고**) · 알림톡 아홉→**여섯 + 손으로 넷** ·
  마이페이지가 화면 하나에서 **쌓이는 단계 목록**으로 · 닉네임을 **손님이 정한다**.
  🔴 **003과 004가 어긋나 보이면 004가 이긴다.** 003 머리말에 대조표가 있다.
...
- 🔴 **정본은 이 레포다**(2026-09-05 소유자). 지식베이스 레포
  ...
  그대로 쓰지 말고 풀어서 설명한다.
  🔴 **예외 둘**(소유자가 명시적으로 지정했다).
  **① 손님에게 나가는 안내 문구** — 정본은 지식베이스 `오프라인 프로그램/8_알림톡/`이다.
  카카오 심사에 올린 글자가 거기 있고, 우리 DB는 그것을 옮겨 심은 것이다.
  **② 현장 폼과 좋아요 폼의 문항** — 정본은 `오프라인 프로그램/7_폼/`이다.
  ⚠️ **로컬 체크아웃이 자주 뒤처진다.** 지식베이스를 볼 때는 `git pull`을 먼저 하거나
  `gh api`로 원격에서 읽는다 — 안 그러면 없는 폴더를 없다고 보고하게 된다.
...
🔴 손님이 받는 주소는 **`/me/[token]` 하나다**(#32에서 만들었다. 알림톡 링크가 그리로 간다).
단계별 주소 `/pre/[token]`·`/q/[token]`은 #30에서 지웠다.
🔴 **주소를 바꾸지 말 것** — 카카오 심사를 통과한 문구는 변수 자리 말고 못 고친다.
주소가 바뀌면 여섯 개를 다시 심사받는다.
...
🔴 `/admin` 현황판은 #34에서 다시 세웠다(목록·상세 서랍·발송·닉네임·마케팅·문구 관리).
아래 규칙을 그대로 지킨다 — 참가자 20명의 이름·연락처·생년월일이 전부 보이는 화면이다.
...
- **`src/lib/event.ts`** — 행사의 확정 사실. `EVENT`(날짜·시간·장소·정원·가격·나이) ·
  ...
  🔴 **여기 값 셋이 지금 사실과 다르다**(2026-09-11 대조). 고칠 때 딸린 곳까지 함께 본다.
  **장소** — 코드는 `합정`, 확정은 **서울 강남구 봉은사로 206 3층**(언주역, 2026-09-07 대관).
  구조화 데이터에도 주소가 따로 박혀 있고, 알림톡 장소 변수도 이 값을 가져간다.
  **환불** — 코드는 3단이고 마지막이 「환불이 어렵습니다」, 확정은 **4단**이며 불참해도
  3,900원을 돌려준다. 약관 조문과 화면이 어긋나면 안 되므로 셋을 함께 고친다.
  **시간** — 알림톡은 「18:00 (17:30 입장)」 한 줄을 기대하는데 코드에는 시작·종료와
  입장 시각이 따로 있다. 합친 표기를 여기서 파생시킨다.
...
- **`src/lib/notification.ts`** — 알림톡 문구 채우기·발송·기록. 🔴 못 채운 변수가 하나라도
  ...
  🔴 **문구 전문의 정본은 지식베이스** `오프라인 프로그램/8_알림톡/`이다(2026-09-11 소유자 지정).
  `docs/alimtalk-templates.md`는 **지도**다 — 어디로 나가는지와 지켜야 할 여섯을 담고 전문은 없다.
  런타임 값은 DB(`notification_template`)에 있다 — 운영자가 배포 없이 고칠 수 있어야 해서다.
  🔴 **셋이 어긋나면 한 통도 안 나간다** — 카카오에 올린 본문과 DB 본문이 개행 한 칸까지
  같아야 하고, 다르면 그 사실이 접수가 아니라 도달 조회에서만 드러난다.
...
- 🔴 알림톡은 `src/lib/alimtalk.ts` 한 통로로만 나간다. `ALIMTALK_PROVIDER`가 `fake`면
  ...
  ⚠️ 같은 이유로 그 **반대도 막는다** — 대행사를 골랐는데 키가 없으면 조용히 가짜로
  넘어가지 않고 던진다. 안 그러면 스무 명이 안내를 받은 줄 알고 운영자가 기다린다.
  🔴 **대행사는 알리고다**(2026-09-07 대표 지시 · `docs/decisions/004…` §5).
  지금 코드에 남아 있는 뿌리오 구현체(#31)는 **갈아끼울 대상**이고 환경변수 이름도 함께 바뀐다.
  ⚠️ **알림톡과 문자는 서로 다른 서비스다** — 주소도 인증 파라미터 이름도 다르고,
  **성공을 뜻하는 코드값까지 다르다.** 하나로 뭉치면 문자가 전부 실패로 기록된다.
- 🔴 알림톡 문구는 표를 비울 때 함께 지워지고, `tests/helpers/db.ts`가
  **씨앗 마이그레이션을 그대로 다시 돌려** 세운다. 테스트용 문구를 따로 적지 말 것 —
  두 벌이 되면 테스트는 통과하는데 배포하면 깨진다.
  ⚠️ 문구는 **열 개**이고 그중 알림톡이 여섯이다. 씨앗 파일이 `006` 하나가 아니게 되면
  이 헬퍼가 도는 파일 목록도 함께 고쳐야 한다.
