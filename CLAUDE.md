...
**`layout.tsx`** — `INTRO_SEEN_SCRIPT`가 하이드레이션 전에 `<html>`에 속성을 붙이므로
`<html>`의 `suppressHydrationWarning`이 필수다. 지우면 재방문 시 hydration mismatch가 난다.

**히어로 인트로(`/` 전용)** — 로고 입모양 + 줌인이다. **하트를 되살리지 말 것.**
`HomeHero.tsx`의 `STROKE`는 로고의 실제 획을 100×100 viewBox로 옮긴 것이다.
⚠️ **두 끝점이 대칭축(x=50)에 닿으면 안 된다** — 닿는 순간 미러링된 두 획이 맞물려 하트가 된다.
`heart` 문자열로는 안 잡히니 **도형을 직접 렌더해서 확인**할 것.
`prefers-reduced-motion`에서는 `.hero__intro`가 `display:none`이다(globals.css 맨 아래).
