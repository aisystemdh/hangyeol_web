...
> |---|---|
> | 말의 뜻 | `CONTEXT.md` |
> | 설계 근거 (지금 서 있는 것) | `docs/decisions/003-scenario-redesign-2026-09-05.md` |
> | 설계 근거 (앞으로 만들 것) | `docs/decisions/004-aligo-cumulative-mypage-2026-09-11.md` |
...
> ### 🔴 2026-09-11 — 곧 또 바뀐다
>
> `004`가 승인됐다. 아래가 바뀌므로 그때 이 문서를 다시 쓴다.
> **없어질 것**: `/api/alimtalk/result`(알리고는 도달 결과를 밀어 주지 않는다) ·
> 문구 행의 버튼 · 뿌리오 환경변수 넷.
> **새로 생길 것**: 마이페이지의 단계 목록 응답 · 손님이 정하는 닉네임 칸 ·
> 현장 폼과 좋아요·연결 결과 · 기한 임박 자동 발송 배치 · 파기 배치의 30일 단 ·
> 운영자 수정 API · 법률 페이지 둘(`/terms`·`/privacy`).
> ⚠️ **§5.8(환경변수 대조)도 함께 낡는다** — 대행사 키 이름이 전부 바뀐다.
