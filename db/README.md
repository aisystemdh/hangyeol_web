...
| `006_templates.sql` | 알림톡 문구 아홉 개의 **첫 씨앗** + 발송 실패 사유 칸. 이후 수정은 운영자 화면에서 한다 — 이 파일을 따라 고치지 말 것. ⚠️ **씨앗 아홉 개는 2026-09-11 부로 낡았다**(열 개 · 버튼 없음 · 링크 앞 고정값). 새 번호의 파일이 갱신한다 — `docs/decisions/004…` §1. 문구 전문의 정본은 지식베이스 `오프라인 프로그램/8_알림톡/`이다 |
