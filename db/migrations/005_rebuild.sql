...
--
-- 🔴 **`npm run db:migrate`로 돌린다.** 실행기가 파일 전체를 한 트랜잭션으로 감싸므로
--    중간에 깨지면 통째로 되돌아간다. ⚠️ psql로 손수 돌릴 거면 반드시
--    `psql "$URL" --single-transaction -f ...`다 — psql은 기본이 **문장마다 자동 커밋**이라,
--    아래 drop 일곱 줄이 먼저 커밋된 뒤 create에서 실패하면 **되돌릴 것이 아무것도 없다.**
-- ═══════════════════════════════════════════════════════════════
